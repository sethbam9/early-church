...
| **🕸️ Graph** | Understanding relationships and connections between people, works, events, and ideas; finding paths between figures |
...
- Use the **CrossPageNav** icons (🗺 / 🕸️ / 📖) in the entity header to jump to the same entity on a different page.
...
1. Go to **🗺 Map**.
2. Open the **Global Search Overlay** (search bar at top of left panel).
...
**Tip:** Use the **⇄ swap** button to reverse start and end. Use "Use selected as start/end" when you've already clicked a node you want to use.
...
10. Use **CrossPageNav** (🗺) to jump to the Map and see all his locations highlighted on the map.
...
5. **Info tab** — shows the dominant polity group and presence status for this decade.
...
3. The **Content** tab renders the full essay. All `[[entity]]` links are blue/interactive — hover to see a tooltip, click to navigate to that entity's detail.
4. Switch to the **Entities** tab to see every referenced entity grouped by kind (People, Works, Places, etc.). Click any to jump directly to its detail.
5. Use **← Back** to return to the essay after exploring an entity.
6. Click **🗺️ Open in map** in the top bar to move to the Map page with the essay open in the right panel, so you can simultaneously read the essay and pan to the places it discusses.
...
1. Go to **🗺 Map** (or start from Wiki, open the work, then use CrossPageNav 🗺).
...
- **Keyboard search**: From the Wiki entity list, use the Global Search Overlay — arrow keys navigate, Enter selects.
- **Back-stack**: Every entity click pushes to history. Use ← Back freely; ✕ exits to the top-level list.
- **Cross-page continuity**: The selection state persists when switching pages. Click 🗺 on a person in the wiki and the map will be centered on their footprint.
...
- **Presence chips**: Ctrl-click (or just click) multiple presence filter chips to focus on specific attestation quality (e.g., show only "attested" places for the highest-confidence view).
